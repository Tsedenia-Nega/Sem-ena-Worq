...
class TestimonialRepository {
  async create(testimonialData) {
    try {
      // 1. REMOVED THE TITLE CHECK
      // Check if this exact person already left this exact review instead
      const existTestimony = await Testimonial.findOne({
        name: testimonialData.name,
        content: testimonialData.content,
      });

      if (existTestimony) {
        throw new Error("This testimonial has already been submitted.");
      }

      const testimony = new Testimonial(testimonialData);
      return await testimony.save();
    } catch (error) {
      // 2. Updated error message check
      if (error.message === "This testimonial has already been submitted.") {
        throw error;
      }
